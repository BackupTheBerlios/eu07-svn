...
osg::Object::DataVariance getNodeDataVariance(INode *node)
{
	BOOL prop= false;
	BOOL ret= node->GetUserPropBool("dynamic",prop);
	return (prop&&ret ? osg::Object::DYNAMIC : osg::Object::STATIC);
}
/**
...
osg::ref_ptr<osg::MatrixTransform> OSGExp::createHelperObject(osg::Group* rootTransform, INode* node, Object* obj, TimeValue t){
    ...
    if (id == BILLBOARD_CLASS_ID){
		rootTransform->addChild(createBillboardFromHelperObject(rootTransform, node, obj, t).get());
	}
	else if (id == LOD_CLASS_ID){
		rootTransform->addChild(createLODFromHelperObject(rootTransform, node, obj, t).get()); 
	}
	else if (id == SEQUENCE_CLASS_ID){
		rootTransform->addChild(createSequenceFromHelperObject(rootTransform, node, obj, t).get()); 
	}
	else if (id == SWITCH_CLASS_ID){
		rootTransform->addChild(createSwitchFromHelperObject(rootTransform, node, obj, t).get()); 
	}
	else if (id == IMPOSTOR_CLASS_ID){
		rootTransform->addChild(createImpostorFromHelperObject(rootTransform, node, obj, t).get()); 
	}
	...
	if(_options->getExportGeometry() && mesh->numVerts){
		...
		if(_options->getExportTextureCoords()){
			for(unsigned int unit=0; unit<mappingList.size(); unit++ ){
				...
				osg::Texture *tex= dynamic_cast<osg::Texture*>(stateset->getTextureAttribute(unit,osg::StateAttribute::TEXTURE));
				bool flipTv= false;
				if (tex)
				{
					std::string texName= tex->getImage(0)->getFileName();
					flipTv= (texName.find(".dds")!=std::string::npos) || (texName.find(".DDS")!=std::string::npos);
				}
				...
				if(mapChan>0 && mapChan<mesh->numMaps && mesh->maps[mapChan].vnum){
					...
					// If uv.x is bigger than 1 or lower than -1 then set WRAP_S mode on texture to REPEAT.
					if(wrap_s)
						_mtlList->setTextureWrap(stateset.get(), unit, osg::Texture::WRAP_S, osg::Texture::REPEAT);
					// If uv.y is bigger than 1 or lower than -1 then set WRAP_T mode on texture to REPEAT.
					if(wrap_t)
		...
		if(_options->getExportGeometry() && mesh->faces){
			...
			if(_options->getExportTextureCoords()){
				...
				for(int unit=0; unit<numTexUnits[i]; unit++){
					...
					bool flipTv= true;
					osg::Texture *tex= dynamic_cast<osg::Texture*>(state[i]->getTextureAttribute(unit,osg::StateAttribute::TEXTURE));
					if (tex)
					{
						std::string texName= tex->getImage(0)->getFileName();
						flipTv= (texName.find(".dds")!=std::string::npos) || (texName.find(".DDS")!=std::string::npos);
					...
					if(mapChan>0 && mapChan < mesh->numMaps && mesh->maps[mapChan].vnum && mesh->maps[mapChan].tf){
						...
						UVVert uv1 = mesh->maps[mapChan].tv[ mesh->maps[mapChan].tf[j].t[0] ];
						UVVert uv2 = mesh->maps[mapChan].tv[ mesh->maps[mapChan].tf[j].t[1] ];
						UVVert uv3 = mesh->maps[mapChan].tv[ mesh->maps[mapChan].tf[j].t[2] ];
			...
			if(_options->getExportVertexColors() && mesh->getNumVertCol() > 0 && mesh->vcFace){
				...
				Point3 c1 = mesh->vertCol[ mesh->vcFace[j].t[0] ];
				Point3 c2 = mesh->vertCol[ mesh->vcFace[j].t[1] ];
				Point3 c3 = mesh->vertCol[ mesh->vcFace[j].t[2] ];
...
osg::ref_ptr<osg::Geode> OSGExp::createParticleSystemGeometry(osg::Group* rootTransform, INode* node, Object* obj, TimeValue t){
	...
	if(obj->ClassID() == Class_ID(RAIN_CLASS_ID,0) || obj->ClassID() == Class_ID(SNOW_CLASS_ID,0) ){
		...
		osg::ref_ptr<osg::StateSet> stateset = _mtlList->getStateSet(node->GetMtl());
		osg::Vec4 diffuse;
		osg::ref_ptr<osg::Texture2D> osgTex;
		int texUnit = 0;
		...
		if(stateset.valid()){
			...
			osgTex = dynamic_cast<osg::Texture2D*>(_mtlList->getTexture(stateset.get(), texUnit).get());
		}
...
//		ps->setDefaultAttributes(osgTex.get(), true, false, texUnit);
		if (osgTex.valid() && osgTex->getImage())
		ps->setDefaultAttributes(osgTex->getImage()->getFileName(), true, false, texUnit);
