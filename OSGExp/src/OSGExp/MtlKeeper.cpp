 ...
 */
BOOL MtlKeeper::addMtl(Mtl* maxMtl, Options* options, TimeValue t){ 
...
osg::ref_ptr<osg::StateSet> MtlKeeper::convertMaterial(Mtl* maxMtl, Options* options, TimeValue t){
	...
	// Is this a shell material
	if (maxMtl->ClassID() == Class_ID(BAKE_SHELL_CLASS_ID, 0))
		return convertShellMaterial(maxMtl, options, t);
...
//	if(maxMtl->GetSubTexmap(ID_OP) && maxMtl->SubTexmapOn(ID_OP) && maxMtl->GetSubTexmap(ID_OP)!=maxMtl->GetSubTexmap(ID_DI)){
...
/**
 * This method will convert a MAX shell material, and return a
 * corresponding osg::StateSet.
 */
osg::ref_ptr<osg::StateSet> MtlKeeper::convertShellMaterial(Mtl* maxMtl, Options* options, TimeValue t){
	...
	// Convert the baked material.
	osg::ref_ptr<osg::StateSet> stateset = convertMaterial(bakedMtl, options, t);

	// Set the self illumination (emmision) color to white on the baked material.
	if(options->getWhitenSelfIllum())
		setSelfIllumColorToWhite(stateset.get());
...
osg::ref_ptr<osg::Material> MtlKeeper::createMaterial(StdMat* stdMtl, Options* options, TimeValue t){
	...
	float shininess = stdMtl->GetShininess(t)*128.0f;
	...
	osgMtl->setSpecular(osg::Material::FRONT_AND_BACK, 
						osg::Vec4(0,0,0,1));
//	osgMtl->setSpecular(osg::Material::FRONT_AND_BACK, 
//						osg::Vec4(spec.r, spec.g, spec.b, alpha));
	osgMtl->setEmission(osg::Material::FRONT_AND_BACK,
						osg::Vec4(self.r, self.g, self.b, alpha));
	osgMtl->setShininess(osg::Material::FRONT_AND_BACK,shininess); 
...
osg::ref_ptr<osg::Texture> MtlKeeper::convertBitmapMap(Mtl* maxMtl, Texmap* tmap, float blendAmount, osg::StateSet* stateset, Options* options, TimeValue t){
	...
	if(osgTex.valid()){
		...
		else if(options->getWhitenTexMat())
			setDiffuseColorToWhite(stateset);
	}	
	return osgTex;
}
...
osg::ref_ptr<osg::Texture2D> MtlKeeper::createTexture2D(Mtl* maxMtl, Bitmap* bmap, std::string name, osg::StateSet* stateset, Options* options, TimeValue t){
	...
	osgTex->setUseHardwareMipMapGeneration(false);
	setTexComp(osgTex.get(), options);

	// If this bitmap has an alpha channel then add an alpha blend function to stateset.
	if(bmap->HasAlpha()){
		std::string n(maxMtl->GetName().data());
		if (n.substr(0,5)=="Blend")
			addAlphaBlendFunc(stateset);
		else
			addAlphaTestFunc(stateset);
	}
	...
	osgTex->setUnRefImageDataAfterApply(true);
...
osg::ref_ptr<osg::Image> MtlKeeper::createImage(Mtl* maxMtl, Bitmap* bmap, std::string name, Options* options, TimeValue t){
	...
	std::string ddsName= name.substr(0,name.find_last_of('.'));
//	image->setFileName("images\\"+ddsName+".dds");
	image->setFileName("images\\"+name);
	...
	if(options->getQuickView() || options->getIncludeImageDataInIveFile()){
		...
		if(data){
			...
			for(int h=height-1; h>=0; h--){
				bmap->GetLinearPixels(0, h, width-1, in);
				BMM_Color_64 *i = in;
...
std::string MtlKeeper::createMapName(std::string oldname, Options* options){
	...
	{
		sprintf(buf,"%s%s", f, e);
		options->setCopyTexture(true);
		options->setWriteTexture(false);
	}
	else
		sprintf(buf,"%s.%s", f, texFormat);
...
//		diffuse = osgMtl->getDiffuse(osg::Material::FRONT_AND_BACK);
		osgMtl->setDiffuse( osg::Material::FRONT_AND_BACK, osg::Vec4(1, 1, 1, 1));
		osgMtl->setAmbient( osg::Material::FRONT_AND_BACK, osg::Vec4(1, 1, 1, 1));
		osgMtl->setSpecular( osg::Material::FRONT_AND_BACK, osg::Vec4(0, 0, 0, 1));
	}
}

/**
 * This method will set the self illumination color of a the
 * material in the given stateset to white. 
 * This is often used when converting a baked shell material.
 */
void MtlKeeper::setSelfIllumColorToWhite(osg::StateSet* stateset){

	osg::Material* osgMtl = (osg::Material*) stateset->getAttribute(osg::StateAttribute::MATERIAL);
	osg::Vec4 selfIllum;

	if(osgMtl){
		selfIllum = osgMtl->getEmission(osg::Material::FRONT_AND_BACK);
		osgMtl->setEmission(osg::Material::FRONT_AND_BACK, osg::Vec4(1, 1, 1, selfIllum.w()));
	}
}
 ...
 */
void MtlKeeper::setTexComp(osg::Texture* tex, Options* options){
	...
	if(comp.compare(std::string(options->getTexComp(0)))==0)
		...
		tex->setInternalFormatMode(osg::Texture::USE_S3TC_DXT1_COMPRESSION);
		...
		tex->setInternalFormatMode(osg::Texture::USE_S3TC_DXT5_COMPRESSION);
}
...
/**
 * This method will add an alpha test function to the given stateset.
 */
void MtlKeeper::addAlphaTestFunc(osg::StateSet* stateset){
	stateset->setAttributeAndModes(new osg::AlphaFunc(osg::AlphaFunc::GREATER,0.5), osg::StateAttribute::ON);
}
