...
nWavStream::Read(char *buf, int num)
{
    ...
    else 
    {
        ...
        while (bytesLeft > 0) 
        {
            ...
            if ((pos + readBytes) > this->dataLen - this->stopOffset)
            {
                readBytes = this->dataLen - this->stopOffset - pos;
            }
            ...
            if (bytesLeft != readBytes) 
            {
                ...
                else 
                {
                    ...
                    this->file->Seek(this->dataStart + this->startOffset, nFile::START);
                }
            }
            bytesLeft -= readBytes;
            buf       += readBytes;
...
nWavStream::Tell(void)
{
    ...
    n_assert(this->file);
    int pos = this->file->Tell() - ( this->dataStart );
    if (pos >= this->dataLen - this->stopOffset) 
    {
        if (this->isLooping) 
        {
            pos = 0 + this->startOffset;
        }
        else
        {
            pos = dataLen - this->stopOffset;
        }
    }
    return pos;
}
...
nWavStream::Seek(int pos)
{
    n_assert(this->isOpen);
    n_assert(this->file);
    n_assert((pos >= 0) && (pos <= this->dataLen - this->stopOffset));

    if ((pos == this->dataLen - this->stopOffset ) && (!this->isLooping)) 
    {
        this->isEof = true;
    } 
    else 
