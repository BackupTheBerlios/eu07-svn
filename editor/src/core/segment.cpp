...
bool __fastcall Segment::Init(osg::Vec3d NewPoint1, osg::Vec3d NewPoint2,
                                float fNewRoll1, float fNewRoll2)
{
    osg::Vec3d dir;
    dir= NewPoint2-NewPoint1;
	dir.normalize();
    return (Segment::Init(NewPoint1,dir,
                        -dir,NewPoint2,
                        fNewRoll1,fNewRoll2,false));
}

bool __fastcall Segment::Init(osg::Vec3d NewPoint1, osg::Vec3d NewCPointOut,
...
//	assert(fLength<=101);
    if (fLength<=0)
    {
//        MessageBox(0,"Length<=0","Segment::Init",MB_OK);
        return false;
    }
...
//    return true;
    ...
    if (fLength>=101)
    {
//        MessageBox(0,"Length>500","Segment::Init",MB_OK);
        return false;
    }
...
osg::Vec3d __fastcall Segment::GetFirstDerivative(double fTime)
{
...
double __fastcall Segment::RombergIntegral(double fA, double fB)
{
    double fH = fB - fA;
...
double __fastcall Segment::GetTFromS(double s)
{
...
osg::Vec3d __fastcall Segment::GetDirection(float fDistance)
{
...
osg::Vec3d __fastcall Segment::FastGetDirection(float fDistance, float fOffset)
{
...
osg::Vec3d __fastcall Segment::GetPoint(float fDistance)
{
    if (bCurve)
    {
        float t= GetTFromS(fDistance);
...
osg::Vec3d __fastcall Segment::FastGetPoint(float t)
{
...
float __fastcall Segment::FastGetTFromS(float s)
{
    if (s>=fLength)
        return 1.0f;
//        s= fLength;
...
/*
bool __fastcall Segment::RenderSwitchRail(const osg::Vec3d *ShapePoints1, const osg::Vec3d *ShapePoints2,
...
*/
/*
int Segment::RenderLoft(nVertexBuffer *vb_dest, 
...
void Segment::CreateLoft(nVertexBuffer* &vb_dest, nIndexBuffer* &ib_dest, nKernelServer *kernelServer,
...
void Segment::MakeCollideShape(nCollideShape *cs, const osg::Vec3d *ShapePoints, int iNumShapePoints,
			double fTextureLength, float s1, float s2, float invQuality)
{
    if (s1<0 || s2>fLength)
        return;
...
/*
bool __fastcall Segment::RenderLoft(const osg::Vec3d *ShapePoints, int iNumShapePoints,
...
bool __fastcall Segment::RenderLoft(const osg::Vec3d *ShapePoints1, const osg::Vec3d *ShapePoints2,
...
bool __fastcall Segment::Render()
{
        osg::Vec3d pt;
...
bool Segment::LoadShapes(ShapesList &sl, std::string &texture, std::string &fileName)
{
	fileName.insert(0,"shapes/");
	try
	{
