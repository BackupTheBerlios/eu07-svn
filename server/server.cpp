...
{
	static std::auto_ptr<Server> self(new Server());
	return self.get();
}
...
struct StartPoint
{
	StartPoint() : trackID(0xFFFFFFFF), owner(0xFFFFFFFF) {}; 
...
#undef DXTRACE_ERR
#define DXTRACE_ERR(x,y) printf("DXTRACE_ERR %s\n",x);

CRITICAL_SECTION g_csPlayerContext;
CRITICAL_SECTION g_csMessage;
...
TCHAR              g_strAppName[256]             = TEXT("Server");
TCHAR              g_strSessionName[MAX_PATH]    = TEXT("Test Session");           // Session name
DWORD              g_dwPort;                                // Port
HRESULT            g_hrDialog;                              // Exit code for app 
BOOL               g_bServerStarted              = FALSE;   // TRUE if the server has started
BOOL               g_RequestExit				 = FALSE;
int				   g_MaxPlayers= 2;
std::vector<StartPoint> g_StartPoints;
unsigned int g_SceneryID=0xFFFFFFFF;
...
void Message(const char *plr, const char *msg)
{
	EnterCriticalSection(&g_csMessage);
	if (!messageWaiting)
	{
...
void Message(PWSTR plr, const char *msg)
{
	EnterCriticalSection(&g_csMessage);
	if (!messageWaiting)
	{
...
typedef std::list< std::auto_ptr<Train> > TrainsList;
...
HRESULT  SendCreatePlayerMsg( Train* pPlayerInfo, DPNID dpnidTarget );
...
//    HKEY    hDPlayServerRegKey;
	...
	try
	{
		file.open("scenery.cfg",std::ios::in);
		unsigned int n= 0;
		file >> n;
		if (n<1000)
		{
			g_StartPoints.resize(n);
			for (unsigned int i=0; i<n; i++)
				file >> g_StartPoints[i].trackID;
			g_MaxPlayers= n;
		}
		file.close();
                                         ...
                                         PVOID pMsgBuffer )
{
    ...
    switch( dwMessageId )
    {
        ...
        case DPN_MSGID_CREATE_PLAYER:
        {
            ...
            Train* pPlayerInfo = new Train;
			//trainsList.push_back(pPlayerInfo);
            //ZeroMemory( pPlayerInfo, sizeof(Train) );
            //pPlayerInfo->lRefCount   = 1;
            pPlayerInfo->dpnidPlayer = pCreatePlayerMsg->dpnidPlayer;

			Message(pdpPlayerInfo->pwszName,"connected");
            ...
            SendWorldStateToNewPlayer( pCreatePlayerMsg->dpnidPlayer, pPlayerInfo);
        ...
        case DPN_MSGID_DESTROY_PLAYER:
        {
            ...
            Train* pPlayerInfo = (Train*) pDestroyPlayerMsg->pvPlayerContext;

            // Ignore this message if this is the host player
            if( pPlayerInfo == NULL )
                break; 

			Message(pPlayerInfo->strPlayerName,"disconnected");
...
//            PLAYER_RELEASE( pPlayerInfo );  // Release player and cleanup if needed
			pPlayerInfo->sp->owner= 0xFFFFFFFF;
        ...
        case DPN_MSGID_RECEIVE:
        {
            ...
            Train* pPlayerInfo = (Train*) pReceiveMsg->pvPlayerContext;
			union
			{
				void					*data;
...
//    DXUtil_ConvertGenericStringToWide( wstrSessionName, g_strSessionName );
	...
	dpnAppDesc.dwMaxPlayers		= g_MaxPlayers+1;
...
//    SendMessage( GetDlgItem(hDlg, IDC_PLAYER_LIST), LB_RESETCONTENT, 0, 0 );
    ...
    for( DWORD i = 0; i<dwNumPlayers; i++ )
    {
		...
		printf("DPNID: 0x%0.8x (%s)\n", pPlayerInfo->dpnidPlayer, pPlayerInfo->strPlayerName);
...
//-----------------------------------------------------------------------------
HRESULT SendCreatePlayerMsg( Train* pPlayerAbout, DPNID dpnidTarget )
{
	...
	msgCreatePlayer.trackID= pPlayerAbout->trackID;
	msgCreatePlayer.dist= pPlayerAbout->dist;
...
//-----------------------------------------------------------------------------
HRESULT SendWorldStateToNewPlayer( DPNID dpnidNewPlayer, Train* pPlayerInfo )
{
    HRESULT hr;
    DWORD dwNumPlayers = 0;
    DPNID* aPlayers = NULL;
	...
	for (unsigned int i=0; i<g_StartPoints.size(); i++)
		if (g_StartPoints[i].owner==0xFFFFFFFF)
		{
			g_StartPoints[i].owner= dpnidNewPlayer;
			pPlayerInfo->trackID=msgSetID.trackID= g_StartPoints[i].trackID;
			pPlayerInfo->dist=msgSetID.dist= 0;
			pPlayerInfo->sp= &g_StartPoints[i];
