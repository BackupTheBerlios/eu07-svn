...
nNebulaScriptClass(nGlServer, "ngfxserver");
...
//-------------------------------------------------------------------
nGlServer::nGlServer()
         : ref_is(kernelServer,this), ref_devdir(this), frameNumber(0), mouseX(0) ,mouseY(0), 
		 updateTime(0), cullTime(0), drawTime(0)
{
...
//	osgDB::Registry::instance()->setDataFilePathList("textures");
		//.push_back("texture");
//	osgDB::getDataFilePathList().push_back("textures");
//	osgDB::getDataFilePathList().push_front(texPath);
//	osgDB::getDataFilePathList().clear();
	//osgDB::PushAndPopDataPath tex_path("texture");

	osgDB::ReaderWriter::Options *defaultOptions= new osgDB::ReaderWriter::Options("noTexturesInIVEFile");
	//defaultOptions->setObjectCacheHint(osgDB::ReaderWriter::Options::CACHE_ALL);
	
	union
	{
		unsigned int i;
		osgDB::ReaderWriter::Options::CacheHintOptions e;
	} tmp;												//UGLY HACK :(
	tmp.i= osgDB::ReaderWriter::Options::CacheHintOptions::CACHE_IMAGES|
		osgDB::ReaderWriter::Options::CacheHintOptions::CACHE_ARCHIVES|
//		osgDB::ReaderWriter::Options::CacheHintOptions::CACHE_NODES;
		osgDB::ReaderWriter::Options::CacheHintOptions::CACHE_OBJECTS;
//	defaultOptions->setObjectCacheHint(tmp.e);
	defaultOptions->setObjectCacheHint(osgDB::ReaderWriter::Options::CACHE_ALL);

	osgDB::Registry::instance()->setOptions(defaultOptions);
	...
	root= new osg::Group();
	mainRoot->addChild(root.get());
	dynamicModelsRoot= new osg::Group();
	root->addChild(dynamicModelsRoot.get());
	particlesRoot= new osg::Geode;
	root->addChild(particlesRoot.get());

	sceneView= new osgUtil::SceneView;
	sceneView->setInitVisitor(new osgUtil::GLObjectsVisitor());
//	sceneView->setDefaults(osgUtil::SceneView::NO_SCENEVIEW_LIGHT);
	sceneView->setDefaults(osgUtil::SceneView::NO_SCENEVIEW_LIGHT|osgUtil::SceneView::COMPILE_GLOBJECTS_AT_INIT);
//	osgUtil::SceneView::
//		sceneView->setCullingMode(osg::CullSettings::NO_CULLING);
//	sceneView->setClearColor(osg::Vec4(0,1,1,1));
//	sceneView->setSceneData(root.get());
	sceneView->setSceneData(mainRoot);
	sceneView->setComputeNearFarMode(osg::CullSettings::DO_NOT_COMPUTE_NEAR_FAR);

	frameStamp= new osg::FrameStamp();

	databasePager= osgDB::Registry::instance()->getOrCreateDatabasePager();
...
//		globalStateSet->setMode( GL_CULL_FACE, osg::StateAttribute::OFF );        
...
//		alphafunc->setFunction(osg::AlphaFunc::ALWAYS,0.0f);
		...
		lightsource->setReferenceFrame(osg::LightSource::ReferenceFrame::RELATIVE_RF);
        lightsource->setStateSetModes(*globalStateSet, osg::StateAttribute::ON);

        root->addChild(lightsource);

	atmoRoot= new osg::MatrixTransform(osg::Matrixd::identity());
//	atmoRoot->setNodeMask(0xFFFFFFFF);
//	sceneView->set
	mainRoot->addChild(atmoRoot.get());
//	root->addChild(atmoRoot.get());

//	osg::MatrixTransform *skyDome= new osg::MatrixTransform(osg::Matrixd::identity());
//	root->addChild(skyDome);
//	skyDome->addChild(osgDB::readNodeFile("models.skydome.ive"));
//	skyDome->setReferenceFrame(osg::Transform::ReferenceFrame::ABSOLUTE_RF);
//	atmoRoot->setReferenceFrame(osg::Transform::ReferenceFrame::ABSOLUTE_RF);

	osg::StateSet *atmo_ss= new osg::StateSet;

	atmo_ss->setAttributeAndModes(new osg::Depth(osg::Depth::LESS,0,1,0));
	atmo_ss->setMode( GL_LIGHTING, osg::StateAttribute::OFF );
	atmo_ss->setRenderBinDetails(-10,"RenderBin");
//    atmo_ss->setMode(GL_DEPTH_TEST, osg::StateAttribute::OFF);
//	atmo_ss->setMode( GL_CULL_FACE, osg::StateAttribute::OFF );        

	atmoRoot->setStateSet(atmo_ss);
	osg::Node *nd= osgDB::readNodeFile("models/skydome.ive");
	atmoRoot->addChild(nd);
//	atmoRoot->setCullingActive(false);
//	atmoRoot->s
//	atmoRoot->set
//	atmo_ss->setRenderBinDetails

//   osg::Projection* projection = new osg::Projection;
  //  projection->setMatrix(osg::Matrix::ortho2D(0,1280,0,1024));
    //projection->addChild(atmoRoot.get());
...
void nGlServer::SetClearColor(float r, float g, float b, float a)
{
	...
	sceneView->setClearColor(osg::Vec4(r,g,b,a));
}
                              ...
                              float minz, float maxz)
{
	...
	sceneView->setProjectionMatrixAsFrustum(minx,maxx,miny,maxy,minz,maxz);
