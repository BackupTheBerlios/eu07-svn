...
//		frameStamp->setReferenceTime(kernelServer->ts->GetFrameTime());
...
//		sceneView->setViewMatrix(getInverseMatrix());
		...
		//updateTime, cullTime, drawTime
//		osg::Timer::instance()->g
		updateTime= kernelServer->ts->GetTime();
		sceneView->update();
		updateTime= kernelServer->ts->GetTime()-updateTime;

		// cull scene 
		cullTime= kernelServer->ts->GetTime();
	    sceneView->cull();
		cullTime= kernelServer->ts->GetTime()-cullTime;

        // draw scene
		drawTime= kernelServer->ts->GetTime();
        sceneView->draw();
		drawTime= kernelServer->ts->GetTime()-drawTime;
...
void nGlServer::LoadSceneGraph(const char *name) 
{
	AddSceneGraph(osgDB::readNodeFile(name)); 
}

void nGlServer::AddSceneGraph(osg::Node *node) 
{
	root->addChild(node); 
//	osg::Vec3d pos= node->getBound().center();
	osg::Vec3d pos(-1000,-1000,95);
	sceneView->setViewMatrixAsLookAt(pos+osg::Vec3d(10,0,2),pos,osg::Vec3d(0,0,1));
}

void nGlServer::SetViewMatix(osg::Matrixd &mat)
{
	sceneView->setViewMatrix(mat);
	atmoRoot->setMatrix(osg::Matrixd::translate(osg::Matrixd::inverse(mat).getTrans()));
}

void nGlServer::SetProjectionMatix(osg::Matrixd &mat)
{
	sceneView->setProjectionMatrix(mat);
}
