...
nNebulaScriptClass(nSimIpol, "nroot");

nChannelServer *nSimIpol::channelServer= NULL;
...
/**
*/
void nSimIpol::Initialize(void)
{
    nSimSound *p = (nSimSound *) this->GetParent();
    if (p && p->IsA(kernelServer->FindClass("nsimsound")))
    {
        p->AddIpol(this);
    }
...
//-------------------------------------------------------------------
bool nSimIpol::Connect(const char *cmd_name)
{
    n_assert(cmd_name);
    ...
    {
        n_printf("nSimIpol::Connect(): Parent object '%s' does not accept command '%s'!\n",
                 p->GetName(), cmd_name);
    }
    return retval;
}
...
//  14-Nov-00   floh    created
//-------------------------------------------------------------------
const char *nSimIpol::GetConnect(void)
{
    if (this->cmd_proto) 
    {
...
//  05-Dec-98   floh    created
//-------------------------------------------------------------------
void nSimIpol::AddKey1f(float time, float x)
{
    nIpolKey key(time, x);
...
//  05-Dec-98   floh    created
//-------------------------------------------------------------------
void nSimIpol::AddKey2f(float time, float x, float y)
{
...
//  05-Dec-98   floh    created
//-------------------------------------------------------------------
void nSimIpol::AddKey3f(float time, float x, float y, float z)
{
...
//  05-Dec-98   floh    created
//-------------------------------------------------------------------
void nSimIpol::AddKey4f(float time, float x, float y, float z, float w)
{
...
//    n_assert(sceneGraph);
  ...
  //  nVisNode::Compute(sceneGraph);
    ...
    //        nChannelContext* chnContext = sceneGraph->GetChannelContext();
      //      n_assert(chnContext);
            float t = channelServer->GetChannel1f(this->localChannelIndex);
