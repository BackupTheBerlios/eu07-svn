...
static void n_setoffset(void*, nCmd*);
static void n_setvolume0(void*, nCmd*);
static void n_setvolume1(void*, nCmd*);
static void n_setvolume2(void*, nCmd*);

//static void n_setstartfile(void*, nCmd*);
//static void n_setstopfile(void*, nCmd*);

static void n_setstartoffset(void*, nCmd*);
static void n_setstopoffset(void*, nCmd*);
//------------------------------------------------------------------------------
/**
    @scriptclass
    nsoundnode
...
n_initcmds(nClass* cl)
{
    ...
    cl->AddCmd("v_setoffset_fff",        'STOF', n_setoffset);
    cl->AddCmd("v_setvolume0_f",		 'SVL0', n_setvolume0);
    cl->AddCmd("v_setvolume1_f",		 'SVL1', n_setvolume1);
    cl->AddCmd("v_setvolume2_f",		 'SVL2', n_setvolume2);
//    cl->AddCmd("v_setstartfile_s",       'SSTR', n_setstartfile);
  //  cl->AddCmd("v_setstopfile_s",        'SSTP', n_setstopfile);
    cl->AddCmd("v_setstartoffset_i",     'SSTR', n_setstartoffset);
    cl->AddCmd("v_setstopoffset_i",      'SSTP', n_setstopoffset);
    cl->EndCmds();
}
...
static
void
n_setvolume0(void* slf, nCmd* cmd)
{
    nSimSound* self = (nSimSound*) slf;
	self->SetVolume0(cmd->In()->GetF());
}
...
static
void
n_setvolume1(void* slf, nCmd* cmd)
{
    nSimSound* self = (nSimSound*) slf;
	self->SetVolume1(cmd->In()->GetF());
}
...
static
void
n_setvolume2(void* slf, nCmd* cmd)
{
    nSimSound* self = (nSimSound*) slf;
	self->SetVolume2(cmd->In()->GetF());
}
...
/*
*/
static void n_setstartoffset(void* slf, nCmd* cmd)
{
    nSimSound* self = (nSimSound*) slf;
	self->startOffset= cmd->In()->GetI();
//	if (self->GetStream())
//		self->GetStream()->SetStartOffset(self->startOffset);
//    self->SetStartFile(cmd->In()->GetS());
}

//------------------------------------------------------------------------------
/*
*/
static void n_setstopoffset(void* slf, nCmd* cmd)
{
    nSimSound* self = (nSimSound*) slf;
	self->stopOffset= cmd->In()->GetI();
//	if (self->GetStream())
//		self->GetStream()->SetStopOffset(self->stopOffset);
//    self->SetStopFile(cmd->In()->GetS());
}
...
*/
bool
nSimSound::SaveCmds(nPersistServer* fs)
{
    bool retval = false;
